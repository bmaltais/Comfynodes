...
class ImageMergeNode:
    ...
    def __init__(self):
        """
        Initializes the ImageMergeNode and pre-loads the FaceMesh model for performance.
        """
        # Performance optimization: Initialize FaceMesh model once to avoid reloading on each execution.
        # This significantly reduces latency when facial correction is enabled.
    ...
    def _find_and_warp_faces(self, original_cv2, updated_cv2):
        """
        Finds and warps faces from the updated image to match the original image, using the pre-initialized FaceMesh model for efficiency.
        """
        try:
            # Performance optimization: Use the pre-initialized FaceMesh model.
